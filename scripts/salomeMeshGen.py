...
# Read ods file
# odsfile = rel('../variables03.ods')
sheetname = 'salome'
df = read_ods(odsfile, sheetname)

for i in range(len(df)):
    # print(df.variable[i]+'='+str(df.value[i]))
    exec(df.variable[i]+'='+str(df.value[i]))
N = int(N)
...
with codecs.open(dirshared+fn+'.idf', 'r', encoding='latin-1') as file:
    lines = file.read()
# write output file
fnu = rel('../domus/')+fn+'_utf8'+'.idf'
...
filename = rel('../salome/mesh01.unv')
